...
# FORMAT: obiroi_cns_<DATE>

mkdir obiroi_cns_$DATE

# Create a subdirectory for the affine registration
# FORMAT: obiroi_cns_<DATE>/affine

mkdir obiroi_cns_$DATE/affine

# Create a subdirectory for the syn registration
# FORMAT: obiroi_cns_<DATE>/syn

mkdir obiroi_cns_$DATE/syn
...
./ANTs/Scripts/antsMultivariateTemplateConstruction2.sh -d 3 -b 1 -c 2  -j $THREADS_AFFINE -i $ITERATIONS_AFFINE -k 1 -f 4x2x1 -s 2x1x0vox -q 1x0x0 -t Affine -m MI -r 1 -o affine_ $ID > >(tee -a stdout-affine-template.txt) 2> >(tee -a stderr-affine-template.txt >&2)
...
mv affine_* obiroi_cns_$DATE/affine
mv stdout-affine-template.txt obiroi_cns_$DATE/affine
mv stderr-affine-template.txt obiroi_cns_$DATE/affine
mv intermediate* obiroi_cns_$DATE/affine
mv rigid obiroi_cns_$DATE/affine
mv ANTs_* obiroi_cns_$DATE/affine
...
cp obiroi_cns_$DATE/affine/affine_template0.nii.gz ./affine_template0.nii.gz
cp obiroi_cns_$DATE/affine/affine_template0.nii.gz obiroi_cns_$DATE/affine_template0.nii.gz
...
./ANTs/Scripts/antsMultivariateTemplateConstruction2.sh -d 3 -b 1 -c 2 -j $THREADS_SYN -i $ITERATIONS_SYN -k 1 -f 8x4x2x1 -s 4x2x1x0vox -q 60x180x40x20 -t SyN -m CC -r 0 -o complete_ -z affine_template0.nii.gz $ID > >(tee -a stdout-syn-template.txt) 2> >(tee -a stderr-syn-template.txt >&2)
...
mv complete_* obiroi_cns_$DATE/syn
mv stdout-syn-template.txt obiroi_cns_$DATE/syn
mv stderr-syn-template.txt obiroi_cns_$DATE/syn
mv intermediate* obiroi_cns_$DATE/syn
mv ANTs_* obiroi_cns_$DATE/syn
...
cp obiroi_cns_$DATE/syn/complete_template0.nii.gz obiroi_cns_$DATE/complete_template0.nii.gz
