...
# FORMAT: obiroi_al_mtc_<DATE>

mkdir obiroi_al_mtc_$DATE

# Create a subdirectory for the affine registration
# FORMAT: obiroi_al_mtc_<DATE>/affine

mkdir obiroi_al_mtc_$DATE/affine

# Create a subdirectory for the syn registration
# FORMAT: obiroi_al_mtc_<DATE>/syn

mkdir obiroi_al_mtc_$DATE/syn
...
mv affine_* obiroi_al_mtc_$DATE/affine
mv stdout-affine-template.txt obiroi_al_mtc_$DATE/affine
mv stderr-affine-template.txt obiroi_al_mtc_$DATE/affine
mv intermediate* obiroi_al_mtc_$DATE/affine
mv rigid obiroi_al_mtc_$DATE/affine
mv ANTs_* obiroi_al_mtc_$DATE/affine
...
cp obiroi_al_mtc_$DATE/affine/affine_template0.nii.gz ./affine_template0.nii.gz
cp obiroi_al_mtc_$DATE/affine/affine_template0.nii.gz obiroi_al_mtc_$DATE/affine_template0.nii.gz
...
./ANTs/Scripts/antsMultivariateTemplateConstruction.sh -d 3 -n 1 -c 2 -j $THREADS_SYN -i $ITERATIONS_SYN -k 1 -w 1 -m 60x180x40x20 -t GR -s CC -r 0 -o complete_ -z affine_template0.nii.gz $ID > >(tee -a stdout-syn-template.txt) 2> >(tee -a stderr-syn-template.txt >&2)
...
mv complete_* obiroi_al_mtc_$DATE/syn
mv stdout-syn-template.txt obiroi_al_mtc_$DATE/syn
mv stderr-syn-template.txt obiroi_al_mtc_$DATE/syn
mv intermediate* obiroi_al_mtc_$DATE/syn
mv ANTs_* obiroi_al_mtc_$DATE/syn
...
cp obiroi_al_mtc_$DATE/syn/complete_template0.nii.gz obiroi_al_mtc_$DATE/complete_template0.nii.gz
...
mv obiroi_al_mtc_$DATE ../results
