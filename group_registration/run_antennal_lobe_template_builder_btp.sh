...
# FORMAT: obiroi_al_btp_<DATE>

mkdir obiroi_al_btp_$DATE

# Create a subdirectory for the affine registration
# FORMAT: obiroi_al_btp_<DATE>/affine

mkdir obiroi_al_btp_$DATE/affine

# Create a subdirectory for the syn registration
# FORMAT: obiroi_al_btp_<DATE>/syn

mkdir obiroi_al_btp_$DATE/syn
...
# We MI (global metric) as the similarity metric and GR (greedy syn) as the transformation model despite the fact that we are using affine registration
# This is becuase the RA transformation model is deprecated and causes a Segmentation Fault during Shape Update to Template Step
# Therefore we keep GR model with max 1 iteration at the 2^2 downsampled resolution to keep only the affine component and allow almost no deformation
# Thus this is for all practical purposes affine registration. Suggested by Philip Cook on the ANTs github page. Refer to https://github.com/ANTsX/ANTs/issues/1641
...
mv affine_* obiroi_al_btp_$DATE/affine
mv rigid_* obiroi_al_btp_$DATE/affine
mv stdout-affine-template.txt obiroi_al_btp_$DATE/affine
mv stderr-affine-template.txt obiroi_al_btp_$DATE/affine
mv *.cfg obiroi_al_btp_$DATE/affine
mv GR* obiroi_al_btp_$DATE/affine
...
cp obiroi_al_btp_$DATE/affine/affine_template.nii.gz ./affine_template.nii.gz
cp obiroi_al_btp_$DATE/affine/affine_template.nii.gz obiroi_al_btp_$DATE/affine_template.nii.gz
...
./ANTs/Scripts/buildtemplateparallel.sh -d 3 -i $ITERATIONS_SYN -m 60x180x40x20 -t GR -s CC -z affine_template.nii.gz -c 2 -j $THREADS_SYN -o complete_  $ID > >(tee -a stdout-syn-template.txt) 2> >(tee -a stderr-syn-template.txt >&2)
...
mv complete_* obiroi_al_btp_$DATE/syn
mv stdout-syn-template.txt obiroi_al_btp_$DATE/syn
mv stderr-syn-template.txt obiroi_al_btp_$DATE/syn
mv *.cfg obiroi_al_btp_$DATE/syn
mv GR* obiroi_al_btp_$DATE/syn
...
cp obiroi_al_btp_$DATE/syn/complete_template.nii.gz obiroi_al_btp_$DATE/complete_template.nii.gz
...
mv obiroi_al_btp_$DATE ../results
