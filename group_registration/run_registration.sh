...
cp ../resampled_data/*.nii.gz ./

## STEP 3: Run the registration

# Run the initial affine registration
./ANTs/Scripts/buildtemplateparallel.sh -d 3 -m 1x0x0 -i 10 -o affine_ -c 2 -j 8 synA647_*.nii.gz > stdout-affline-template.txt 2>stderr-affline-template.txt
...
mv job*.* obiroi_brain_$DATE/affine
...
./ANTs/Scripts/buildtemplateparallel.sh -d 3 -i 10 -m 60x180x40x16 -t GR -c 2 -j 8 -o complete_ -z affine_template.nii.gz TEST_*.nii.gz > stdout-syn-template.txt 2>stderr-syn-template.txt
