...
./ANTs/Scripts/antsMultivariateTemplateConstruction2.sh -d 3 -A 2 -b 1 -c 2  -j $THREADS_AFFINE -i $ITERATIONS_AFFINE -k 1 -f 6x4x2x1 -s 4x2x1x0vox -q 200x100x50x0 -t Affine -m MI -r 1 -o affine_ $ID > stdout-affine-template.txt 2>stderr-affine-template.txt
...
./ANTs/Scripts/antsMultivariateTemplateConstruction2.sh -d 3 -b 1 -c 2 -j $THREADS_SYN -i $ITERATIONS_SYN -k 1 -f 6x4x2x1 -s 4x2x1x0vox -q 60x120x40x20 -t SyN -m CC -r 0 -o complete_ -z affine_template0.nii.gz $ID > stdout-syn-template.txt 2>stderr-syn-template.txt
